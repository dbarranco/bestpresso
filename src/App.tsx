...
export default function App() {
  ...
  else if (page === 'settings' && !data.liveBrew.visible) screen = <SettingsPanel model={data.model} feedback={data.settingFeedback} scale={data.scale} scaleTarePending={data.scaleTarePending} settingsDisabled={data.settingsDisabled} onFeedback={(status, message) => data.showSettingFeedback({ status, message })} onSearchScale={data.searchForScale} onTareScale={data.tareConnectedScale} onClose={() => navigate('home')} />
